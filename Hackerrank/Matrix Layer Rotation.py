...
class Matrix:
    ...
    def append(self, x):
        ...
        z = 0
        i = 0
        lk = len(self.dequeues)
        while i < self.m:
            imp = x[i]
            lle = self.n
            for j in range(z):
                self.dequeues[j].add_front([imp[j]])
            if down:
                self.dequeues[z].add_back(imp[z:lle - z])
            else:
                self.dequeues[z].add_back(imp[z:lle - z], True)
            for j in range(z):
                self.dequeues[j].add_back([imp[lle - j - 1]])
            if z < lk - 1 and down:
                z += 1
            elif z == lk - 1:
                down = False
            if not down and i >= self.m - lk:
                z -= 1
            i += 1
